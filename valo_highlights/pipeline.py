...
def extract_frames(input_dir="segments_fast",
                   ...
                   fps=1):
    os.makedirs(output_dir, exist_ok=True)
    for segdir in glob.glob(f"{input_dir}/*/*"):  # note: segments_fast/label/*.webm
        webm = segdir
...
def prepare_data(audio_file="features/audio_features.parquet",
    ...
    # build segment_id same as step 1 (use video_name)
    lb["segment_id"] = (
      lb["video_name"].str.replace(".webm","") + "_" +
...
def train(data_file="features/train_table.parquet",
    ...
    # drop any non-numeric columns so X is pure float32
    X_df = df.drop(columns=[
        "video_name", "start_time", "end_time", "label", "segment_id", "ocr_text"
    ])
    ...
    # save just the weights, and separately the encoder
    torch.save(model.state_dict(), model_out)
    joblib.dump(le, encoder_out)
    print("Model weights saved to", model_out)
    print("LabelEncoder saved to", encoder_out)
...
if __name__ == "__main__":
    # if you want to re-cut from raw_videos, uncomment the next line:
    # extract_segments()
    #extract_frames()
    #extract_audio()
