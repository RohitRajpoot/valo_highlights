...
class HighlightDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frame  = np.load(os.path.join(FRAME_FEAT_DIR, f"{base}.npy"))
        audio  = np.load(os.path.join(AUDIO_FEAT_DIR, f"{base}.npy"))
...
def collate_fn(batch):
    ...
    # 3) Text
    texts_tensor = torch.stack([t for t in texts_list], dim=0)
...
def train():
    ...
    # 2) Dataset + class weights + sampler
    ...
    class_weights= torch.tensor(w_np, dtype=torch.float32).to(device)
    sample_w     = [class_weights[dataset.label2idx[l]].item() for l in labels_arr]
